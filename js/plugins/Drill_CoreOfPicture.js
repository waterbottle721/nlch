...
Sprite_Picture.prototype.updatePosition = function() {
    ...
    // 保留原方法的功能
    var x = Math.round(picture.drill_COPi_finalTransform_x());
    var y = Math.round(picture.drill_COPi_finalTransform_y());
	
	//this.x = x;
	//this.y = y;
    
    // 适配 $gameScreen.zoomScale() 的新功能
    this.x = x * (1 / $gameScreen.zoomScale());
    this.y = y * (1 / $gameScreen.zoomScale());
};
//==============================
// * 固定帧初始值 - 贴图帧刷新 缩放（覆写）
//==============================
Sprite_Picture.prototype.updateScale = function(){
    var picture = this.picture();
    var scaleX = picture.drill_COPi_finalTransform_scaleX();
    var scaleY = picture.drill_COPi_finalTransform_scaleY();
	
	//this.scale.x = scaleX;
	//this.scale.y = scaleY;
	
	this.scale.x = (1 / $gameScreen.zoomScale()) * scaleX;
    this.scale.y = (1 / $gameScreen.zoomScale()) * scaleY;
};
//==============================
