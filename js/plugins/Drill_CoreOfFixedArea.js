...
Game_Map.prototype.drill_COFA_getCustomPointsByIdWithCondition_Private = function( event_id, def_area_id, condition ){
	...
	if( e == undefined ){ return [] }
	condition['center_x'] = Math.floor(e._x);
	condition['center_y'] = Math.floor(e._y);
...
// 适配QJ-SpawnEvent，强制刷新事件容器
Game_Map.prototype.drill_COFA_forceRefresh = function(){
    // 先清空再重新构建
    this._drill_COFA_tank = [];
    this._drill_COFA_countPrivate = 0;
    this.drill_COFA_updateTank();     // 现在会把所有非空事件都加入 _drill_COFA_tank
    this.drill_COFA_updateEvents();   // 再清理一下已被erase的
};
