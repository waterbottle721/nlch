...
(function ($) {
  function evalText(text) {
    ...
    if (currentScene instanceof Scene_Map) {
      ...
      } else if (Imported.ADRI_InBattleEnemyStatus && currentScene._inBattleEnemyStatusWindow && currentScene._inBattleEnemyStateList && currentScene._inBattleEnemyStatusWindow.active) {
        a = $gameVariables.value(40);

        if (currentScene._inBattleEnemyStateList) {
          item = currentScene._inBattleEnemyStateList.item();
        }
      }
  ...
  // Fix for ADRI_InBattleEnemyStatusWindow
    if (Imported.ADRI_InBattleEnemyStatus) {
    $.alias.Window_InBattleEnemyStateList = Window_InBattleEnemyStateList.prototype.setBattler;

    Window_InBattleEnemyStateList.prototype.setBattler = function (battler) {
      $.alias.Window_InBattleEnemyStateList.call(this, battler);
