...
(function (DotMoveSystem) {
    ...
    (function (FunctionEx) {
        ...
        Game_CharacterBase.prototype.update = function () {
			...
			if(this._isPushing && this._pushingcounter > 0){
				this._pushingcounter -= 1;
			}else{
        ...
        DotMoveSystem.CharacterMover.prototype.eventPushProcess = function () {
            ...
            for (const result of this.checkHitCharactersStepDir(pos.x, pos.y, dir, Game_Event)) {
                ...
                if (event.isPushableEvent()) {
                    if (!(result.collisionLengthX() >= margin && result.collisionLengthY() >= margin))
                        continue;
					$gamePlayer._isPushing = true;
					$gamePlayer._pushingcounter = 20;
					let speed = event._moveSpeed;
					if ($gameSwitches.value(192)) {
					event._moveSpeed = $gamePlayer._moveSpeed;
					}
                    event.mover().dotMoveByDirection(dir);
					event._moveSpeed = speed;
		  ...
		  if(Math.random() > 0.8){
         // event.addCloudDust(0.01, radian, event._offsetX, event._offsetY);	
		            }		  
                }
            }
        };
...
Game_Event.prototype.initialize = function(mapId, eventId) {
    ...
    for (var i = 0; i < notedata.length; i++) {
        var line = notedata[i];
        if (line.match(/<(?:pushableEvent)>/i)) {
            this._pushableEvent = true;
            break; 
        }
    }
    const values = this.getAnnotationValues ? this.getAnnotationValues(0) : null;
    if (values && values.PushableEvent) {
        this._pushableEvent = true;
    }
};
        ...
        Game_CharacterBase.prototype.smartJump = function (xPlus, yPlus, baseJumpPeak = 10, through = false) {
			...
			baseJumpPeak = Math.min(10 + distance / 2, 20); 
            this._jumpPeak = baseJumpPeak;
