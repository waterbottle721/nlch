...
Drill_SLS_GoodsWindow.prototype.drawItem = function(index) {
	...
	var pricePos = rect.x + rect.width - priceWidth;		//价格位置x
    ...
    this.drawIcon(item_data.iconIndex, rect.x + 80, itemVertical - 50); //绘制顶部图标
	this.drawTextEx(item_data.description, rect.x + 210, itemVertical - 35, rect.width - priceWidth);
	var textWidth = this.textWidth(item_data.name) / 2;
    this.drawItemNameWithoutIcon(item_data, rect.x - textWidth + 43, itemVertical, rect.width - priceWidth);
	var fontSizeBackup = this.contents.fontSize; //备份字体大小
	if( store_str < 1 ){
	this.drawTextEx('\\c[115]\\{売切 ', pricePos, rect.y + 20, priceWidth, 'right');
	} else {
	this.contents.fontSize += 6;
    this.drawTextEx('\\fs[26]残り\\fs[34]' + store_str, pricePos, rect.y + 8, priceWidth, 'right');
	this.contents.fontSize = fontSizeBackup;
	}
	if( this.drill_SLS_isSoldOut(index) == false ){
		this.contents.fontSize += 6;
		this.drawText(price + 'G', pricePos, rect.y + 64, priceWidth, 'right');	//没有库存后，不显示价格
		this.contents.fontSize = fontSizeBackup;
	}
