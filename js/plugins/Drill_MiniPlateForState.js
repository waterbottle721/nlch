...
　　var DrillUp = DrillUp || {}; 
	...
	/*----------------状态---------------*/
	DrillUp.g_MPFS_list_length = 500;
	DrillUp.g_MPFS_list = [];
...
if( Imported.MOG_ActorHud ){
	...
	Actor_Hud.prototype.update = function() {
		...
		if( this._battler == undefined ){ return; }
		if( this._state_icons == undefined || this._state_icons.length == 0 ){ return; }
		...
		var ww = 0;
		var hh = 32;
		var icon = this._state_icons[0];
		xx = icon.x;
		yy = icon.y;
		ww = this._state_icons.length * 32;
...
Drill_MPFS_Window.prototype.drill_updateMessage = function(){
    ...
    var context_list = [];
    for (var i = 0; i < bean['_drill_state_list'].length; i++) {
    var stateId = bean['_drill_state_list'][i];
    var index = stateId - 1;
    var state = $dataStates[stateId];
    if (!state) continue;
    
    var icon_index = state.iconIndex;
    ...
    if (enabled) {
        // 按换行拆分，并过滤掉空字符串（去除首尾和中间仅有空格的情况）
        var lines = description.split("\n").filter(function(line) {
            return line.trim() !== "";
        });
        if (lines.length > 0) {
            // 第一行添加图标标记
            context_list.push("\\{\\i[" + icon_index + "]\\}" + lines[0]);
            // 后续行依次加入（不会重复添加图标）
            for (var j = 1; j < lines.length; j++) {
                context_list.push(lines[j]);
                }
            }
        }
    }
	...
	if( context_list.length == 0 ){
		if( DrillUp.g_MPFS_default_enable == true ){			
            let textArray = window.systemFeatureText && window.systemFeatureText.noStatusEffects;
            if (!textArray) textArray = "No status effects at all!";
            let text = Array.isArray(textArray) ? textArray.join("\n") : (textArray ?? "");
                text = "\\px[15]\\py[10]\\fs[22]" + text;			
			context_list.push( text );
		}
	}
