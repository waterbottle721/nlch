...
Game_Map.prototype.scrollDown = function(distance){
  if(this.isLoopVertical()){
    ...
    if (this._parallaxSy === 0) this._parallaxY = this._displayY; // changed by PRTN
  }
};
...
Game_Map.prototype.scrollLeft = function(distance){
  if(this.isLoopHorizontal()){
    ...
    if (this._parallaxSx === 0) this._parallaxX = this._displayX; // changed by PRTN
  }
};
...
Game_Map.prototype.scrollTowards = function(chara, speed, frames) {
  ...
  var centerY = this._tDisplayY + this.screenTileY() / 2;
    // changed
    if (chara == $gamePlayer) {
      var centerSlideX = $gameVariables.value(158);
      var centerSlideY = $gameVariables.value(159);
      centerX += centerSlideX;
      centerY += centerSlideY;
    }
    // -------
  if (!this.isLoopHorizontal()) {
