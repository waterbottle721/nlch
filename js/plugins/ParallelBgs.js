...
AudioManager.fadeOutBgsByLine = function(time, lineIndex) {
    var prevIndex = this._bgsLineIndex;  // 保存当前线路索引
    this._bgsLineIndex = lineIndex;      // 设置为指定线路
    if (this._allBgsBuffer[lineIndex]) { // 检查该线路是否有BGS
        this._allBgsBuffer[lineIndex].fadeOut(time);
    }
    this._bgsLineIndex = prevIndex;      // 恢复原来的线路索引
};

AudioManager.fadeInBgsByLine = function(time, lineIndex) {
    var prevIndex = this._bgsLineIndex;  // 保存当前线路索引
    this._bgsLineIndex = lineIndex;      // 设置为指定线路
    if (this._allBgsBuffer[lineIndex]) { // 检查该线路是否有BGS
        this._allBgsBuffer[lineIndex].fadeIn(time);
    }
    this._bgsLineIndex = prevIndex;      // 恢复原来的线路索引
};

AudioManager.stopBgsByLine = function(lineIndex) {
    var prevIndex = this._bgsLineIndex;  // 保存当前线路索引
    this._bgsLineIndex = lineIndex;      // 设置为指定线路
    if (this._allBgsBuffer[lineIndex]) { // 检查该线路是否有 BGS
        this._allBgsBuffer[lineIndex].stop();
        this._currentAllBgs[lineIndex] = null; // 重置当前 BGS
    }
    this._bgsLineIndex = prevIndex;      // 恢复原来的线路索引
};

AudioManager.fadeOutAllBgs = function(time) {
    // 遍历所有 BGS 渠道
    this.iterateAllBgs(function(bgs) {
        if (bgs && typeof bgs.fadeOut === "function") {
            bgs.fadeOut(time);
        }
    });
};

AudioManager.switchBgsInstrument = function(fadeOutIndex, fadeInIndex) {
    // 保存当前线路索引
    const prevIndex = this._bgsLineIndex;

    // 取出要淡出的 BGS 数据，并检查有效性
    const outData = this._currentAllBgs[fadeOutIndex];
    if (!outData || !outData.name) {
        //console.warn(`switchBgsInstrument: no BGS playing on line ${fadeOutIndex}`);
        return;
    }
    let bgsAVolume = outData.volume;
    let bgsAName   = outData.name;

    // 额外针对特定曲目做调整
    if (bgsAName === "それはそれとして（へたっぴリコーダー）") {
        bgsAVolume = 90;
    }

    // 取出要淡入的 BGS 数据，并检查有效性
    const inData = this._currentAllBgs[fadeInIndex];
    if (!inData || !inData.name) {
       // console.warn(`switchBgsInstrument: no BGS playing on line ${fadeInIndex}`);
        return;
    }
    const bgsBName = inData.name;

    // 执行淡出
    this.fadeOutBgsByLine(1, fadeOutIndex);

    // 切换系统线路
    $gameSystem.setBgsLine(fadeInIndex);

    // 针对同一首曲子淡入音量做微调
    if (bgsBName === "それはそれとして（へたっぴリコーダー）") {
        bgsAVolume = Math.max(0, bgsAVolume - 30);
    }
    ...
    this.fadeInBgsByLine(1, fadeInIndex);
...
AudioManager.stopBgsByLine = function(lineIndex) {
    var prevIndex = this._bgsLineIndex;  // 保存当前线路索引
    this._bgsLineIndex = lineIndex;      // 切换到指定线路
    if (this._allBgsBuffer[lineIndex]) { // 如果该线路有 BGS
        this._allBgsBuffer[lineIndex].stop();      // 立即停止播放
        this._currentAllBgs[lineIndex] = null;       // 清空当前 BGS 信息
    }
    this._bgsLineIndex = prevIndex;      // 恢复原来的线路
};
