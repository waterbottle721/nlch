 ...
 * 1. 自动赋予状态条件
 * <AS上限HP:（HPの割合[百分率]）>
 *     当HP(比例)超过（>=）指定值时、赋予目标状态。
 * <AS下限HP:（HPの割合[百分率]）>
 *     当HP(比例)低于（<=）指定值时、赋予目标状态。
 * <AS上限MP:（MPの割合[百分率]）>
 *     当MP(比例)超过（>=）指定值时、赋予目标状态。
 * <AS下限MP:（MPの割合[百分率]）>
 *     当MP(比例)低于（<=）指定值时、赋予目标状态。
 * <AS上限TP:（TPの割合[百分率]）>
 *     当TP(比例)超过（>=）指定值时、赋予目标状态。
 * <AS下限TP:（TPの割合[百分率]）>
 *     当MP(比例)低于（<=）指定值时、赋予目标状态。
 * <AS武器装備:（武器ID）>
 *     装备指定武器时、赋予目标状态。
 ...
 *     当指定的开关为ON时、赋予目标状态。
 * <AS計算式:（JS計算式）>
 *     当指定的JavaScript公式为true时、赋予目标状态。
 * <AS並び順:（インデックス）>
 *     排列顺序(1~ )与指定值一致的期间、赋予目标状态。
 *
 * 如果要在公式中使用不等号，请编写如下。
 * < → &lt;
 * > → &gt;
 * 例：<AS計算式:\v[2] &gt; 1> // 如果变量[2]大于 1
 *
 * 2. 自动赋予状态的目标
 * <ASアクター:（角色ID）>
 *     仅将状态自动赋予的目标设置为指定的角色。
 *     在没有指定id的情况下，对所有的角色都有效。
 *     如果要指定多个角色，请用逗号分隔数值。
 * 例：<ASアクター:1,2,3>
 *
 * <AS敵キャラ:（敌人ID）>
 *     仅将状态自动赋予的目标设置为指定的敌人。
 *     在没有指定id的情况下，对所有的敌人都有效。
 *     如果想要指定多个敌人时，用逗号分隔数值。
 * 例：<AS敵キャラ:1,2,3>
 *
 * 如果指定了多个条件，则只有在满足所有条件时才会赋予状态。
 *
 * 脚本
 * 在启用自动赋予状态时显示赋予消息 
 * $gameSystem.automaticStateAddMessage = true;
 *
 * 如果不想在启用自动赋予状态时显示赋予消息 
