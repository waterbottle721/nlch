...
Game_QJBulletMZ.prototype.refreshBox = function(justMakeData) {
    ...
    if (body[0]=='C') {
        ...
        let w = body[1]*this.scaleX*(0.5-(body[3]==undefined?this.anchorX:body[3]));
        let h = body[1]*this.scaleY*(0.5-(body[4]==undefined?this.anchorY:body[4]));
...
Game_QJBulletMZ.prototype.update = function (ifInit) {
    ...
    //=======================
	// 时停检测
	//if($gameSystem._ZzyTWFTheWorlding && !this.data.immuneTimeStop) return false;
...
Sprite_QJBulletMZ.prototype.loadBitmap = function() {
    ...
    if (typeof img === "string"||img[0]==='P') {
        ...
        let bitmap = ImageManager.loadSystem('IconSet_large');
        this.setBitmap(bitmap);
    } else if (img[0]==='C') {
...
if (true) {
    ...
    model1.initialRotationExtra = ((value,event,pos)=>{//传入的是画面坐标
        if (typeof value !== 'number') {
            ...
            switch(value[0]) {
                ...
                case 'M' :{
                    if (isMV) {
                        value[0] = QJ.calculateAngleByTwoPointAngle(pos[0],pos[1],mouseX / $gameScreen.zoomScale(),mouseY / $gameScreen.zoomScale());addValue = value[1]?value[1]:0;
                    } else {
    ...
    model1.x = ((value,event)=>{
        ...
        switch(value[0]) {
            ...
            case 'M':{
                addValue = value[1]?value[1]:0;
                value = isMV?mouseX / $gameScreen.zoomScale():TouchInput.x;
                break;
            }
            case 'E':{
                addValue = value[2]?value[2]:0;
    ...
    model1.y = ((value,event)=>{
        ...
        switch(value[0]) {
            ...
            case 'M':{
                addValue = value[1]?value[1]:0;
                value = isMV?mouseY / $gameScreen.zoomScale():TouchInput.y;
                break;
            }
            case 'E':{
                addValue = value[2]?value[2]:0;
    ...
    model1.img = ((value)=>{
        if (typeof value === 'object'&&value[0]==='C') {
			...
			let spriteName = sprite._drill_ESE_bitmap._url.replace(/\.png$/, '');
            value=value.concat([spriteName,sprite._frame.x,sprite._frame.y,sprite._frame.width,sprite._frame.height]);
...
QJ.calculateAnnotation = function(event) {
    ...
    if (page) {
        let i = 0;
        while (page.list[i] && (page.list[i].code === 408 || page.list[i].code === 108)) {
            content += page.list[i].parameters[0] + '\n';
            i++;
        }
    }
    return content;
