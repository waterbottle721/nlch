...
Game_CharacterBase.prototype.updateFootSteps = function() {
    ...
    // 计算当前位置与上次脚印位置之间的距离
    var dx = this._x - this._footSteps[4];
    var dy = this._y - this._footSteps[5];
...
Game_CharacterBase.prototype.needCreateFootSteps = function() {
    ...
    var distanceThreshold = 0.5; // 可调整的阈值
    var dx = Math.abs(this._x - this._footSteps[1]);
    var dy = Math.abs(this._y - this._footSteps[2]);
    // 如果 x 或 y 方向上的移动超过阈值，则需要创建脚印
...
FootStepsSprites.prototype.updateDuration = function(i) {
	...
	if($gameSwitches.value(198)){
	this._footsprites[i].opacity -= 0.1; 
	}else {
