...
Game_Variables.prototype.setValue = function(variableId, value) {
    ...
    for(var i=0; i < DrillUp.g_GVa_variable_id.length; i++){
        if (DrillUp.g_GVa_variable_id[i] == variableId){
            DrillUp.global_GVa_variableTank[variableId] = value;
...
Game_System.prototype.set_day_phase = function() {
	if (this.hour() >= 23 || this.hour() < 7) {this._time_data[1] = 3} // 睡眠时间
	else if (this.hour() >= 19) {this._time_data[1] = 2} //夜间时间
	else if (this.hour() >= 14 && this.hour() < 18) {this._time_data[1] = 0} // 深渊探索
	else if (this.hour() >= 13 && this.hour() < 14) {this._time_data[1] = 1} // 午休时间
	else if (this.hour() >= 9 && this.hour() < 13) {this._time_data[1] = 5} // 深渊探索
	else if (this.hour() >= 7 && this.hour() < 9) {this._time_data[1] = 4} // 早晨开始
};
