...
(function() {
	...
	//新建方法。用于在标题画面自动随机播放动画
    var _Scene_Title_create = Scene_Title.prototype.create;
    Scene_Title.prototype.create = function() {
        _Scene_Title_create.apply(this, arguments);
        this._apngSprites = []; 
		this.playApngBasedOnSwitches();
    };

    Scene_Title.prototype.playApngBasedOnSwitches = function() {
        var switchToApngMap = {
            400: 'game_title_sis0',
            401: 'game_title_sis1',
            402: 'game_title_sis2'
        ...
        var activeSwitches = [];
        for (var switchId in switchToApngMap) {
            if ($gameSwitches.value(Number(switchId))) {
                activeSwitches.push(switchToApngMap[switchId]);
            }
        }

        if (activeSwitches.length > 0) {
            var randomApng = activeSwitches[Math.floor(Math.random() * activeSwitches.length)];
            this.playApng(randomApng, 2); 
        }
    };

    Scene_Title.prototype.playApng = function(apngName, loopTimes) {
        var apngSprite = SceneManager.tryLoadApngSystem(apngName);
        if (apngSprite) {
            apngSprite.pixiApng.play(loopTimes || 0); // 默认无限循环
            this._apngSprites.push(apngSprite);
            this.addChild(apngSprite);
        }
    };
	
Scene_Title.prototype.removeApng = function(apngName) {
    for (var i = 0; i < this._apngSprites.length; i++) {
        var apngSprite = this._apngSprites[i];
        if (apngSprite.pixiApngOption.name === apngName) {
            console.log(`Removing APNG: ${apngName}`);
            this.removeChild(apngSprite);
            ApngLoader.destroyApng(apngSprite);
            this._apngSprites.splice(i, 1);
            break;
        }
    }
};
    ...
    this.checkApngPlayback();
    };

Scene_Title.prototype.checkApngPlayback = function() {
    if (this._apngSprites[0].pixiApng.__status.loops >= 2) {
        var name = this._apngSprites[0].pixiApng.sprite.pixiApngOption.FileName;
        this.removeApng(name);
        this.playApngBasedOnSwitches();
    }
};
...
// 当 APNG 停止播放时的处理函数
Scene_Title.prototype.update = function(apngSprite) {
    this._apngSprites = this._apngSprites.filter(function(sprite) {
        return sprite !== apngSprite;
    });

    // 检查是否还有其他 APNG 正在播放
    if (this._apngSprites.length === 0) {
        this.playApngBasedOnSwitches();
    }
};*/
