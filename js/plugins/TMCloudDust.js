...
(function() {
  ...
  Game_Map.prototype.addCloudDust = function(x, y, speed, radian, offsetX, offsetY) {
    if (!$gameSystem.isDustEnabled()) return;
    for (var i = 0; i < TMPlugin.CloudDust.MaxDusts; i++) {
      if (!this._cloudDusts[i].exists()) {
        this._cloudDusts[i].setup(x, y, speed, radian, offsetX, offsetY);
        break;
      }
    }
  };
  ...
  Game_Map.prototype.changeDustImage = function(imageName) {
  TMPlugin.CloudDust.DustImage = imageName;
  };
  //-----------------------------------------------------------------------------
  // Game_CloudDust
  //
  ...
  Game_CloudDust.prototype.initialize = function() {
	...
	this._dustImage = TMPlugin.CloudDust.DustImage; 
  };

  Game_CloudDust.prototype.dustImage = function() {
  return this._dustImage;
  };

  Game_CloudDust.prototype.setup = function(x, y, speed, radian, offsetX, offsetY) {
    this._x = +x;
    this._y = +y;
	this._y -= 0.3;
    this._opacity = 180;
  ...
  Game_CharacterBase.prototype.addCloudDust = function(speed, radian, offsetX, offsetY) {
    $gameMap.addCloudDust(this._realX + 0.5, this._realY + 1.0, speed, radian, this._offsetX, this._offsetY);
  };
  ...
  Game_Player.prototype.executeMove = function(direction) {
	var n = Math.floor($gamePlayer._moveSpeed / 6);
	n = Math.max(n,1);
    if (n > 0) {
      if (this.isDashing() && this.canPass(this.x, this.y, direction)) {
        if (direction === 2) {
          var radian = Math.PI * 1.5;
        } else if (direction === 4) {
          var radian = 0;
          ...
          var radian = Math.PI * 1.75;
        } else if (direction === 3) {
          var radian = Math.PI * 1.25;
        } else if (direction === 7) {
          var radian = Math.PI * 0.25;
        } else if (direction === 9) {
          var radian = Math.PI * 0.75;
        } else {
          var radian = Math.PI / 2;
        }
        for (var i = 0; i < n; i++) {
  ...
  if (this._cloudDust.exists()) {
    if (this.bitmap !== ImageManager.loadSystem(this._cloudDust.dustImage())) {
      this.bitmap = ImageManager.loadSystem(this._cloudDust.dustImage());
    }
    this.visible = true;
  ...
  Sprite_CloudDust.prototype.createBitmap = function() {
    this.bitmap = ImageManager.loadSystem(this._cloudDust._dustImage);
    this.anchor.x = 0.5;
    this.anchor.y = 0.5;
    this.z = 3;
  };
  ...
  Spriteset_Map.prototype.reloadCloudDustBitmaps = function() {
  // 遍历所有云尘精灵
  for (var i = 0; i < this._cloudDustSprites.length; i++) {
    // 重新加载位图
    this._cloudDustSprites[i].bitmap = ImageManager.loadSystem(TMPlugin.CloudDust.DustImage);
    }
  };
